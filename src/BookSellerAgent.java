...
public class BookSellerAgent extends Agent {
  ...
  private HashMap<String,Boolean> proposedBooks = new HashMap<>();
  ...
  public void updateCatalogue(final String title, final int price,final int shippingPrice) {
    addBehaviour(new OneShotBehaviour() {
      public void action() {
		catalogue.put(title, new Integer(price + shippingPrice));
		proposedBooks.put(title,false);
		System.out.println(getAID().getLocalName() + ": " + title + " put into the catalogue. Price + the shipping cost = " + price + " + " + shippingPrice);
      }
    } );
  }
	  ...
	  public void action() {
	    ...
	    if (msg != null) {
	      ...
	      if (price != null){
	      	...
	      	if (!proposedBooks.get(title)){

		        reply.setPerformative(ACLMessage.PROPOSE);
		        reply.setContent(String.valueOf(price.intValue()));
		        proposedBooks.put(title,true);
	  ...
	  public void action() {
	    ...
	    if (msg != null) {
	      ...
	      if (price != null) {
	        ...
	        //title not found in the catalogue, so ld to another agent in the meantime (after proposal submission)
